...
DimPlot(object = seurat_integrated,
  ...
  NoLegend()
```
### Does cell cycle play a role in clustering
...
```{r}
# do clusters segregate by mitochondrial gene ratio
DimPlot(object = seurat_integrated,
        label = TRUE,
        split.by = "mitoFr") + NoLegend()
```
Plot above shows cluster 0  has most (~ 50% cells of cluster 0) of the 'high' mitochondrial fraction cells, we can regress out this variation, if uninteresting to the experiment

## Redo of post QC 
### Regressing out the mitochondrial gene ratio as uninteresting variation

To keep the possibility of clustering the cells with mitochondrial gene content regressed out or not regressed out, we will repeat the analysis beginning with creating a split suerat object but named to explicitly indicate that the mitochindrial gene ratio factor has been regressed out.

Since we have two samples in our dataset, we want to keep them as separate objects and transform them as that is what is required for integration. We will first split the cells in seurat_phase object into “Control” and “treated”:

```{r}
# split seurat object by condition for sctransform
split_seurat_noMito <- SplitObject(seurat_phase,split.by = "sample")
split_seurat_noMito <- split_seurat[c("ctrl", "treat")]
```
SCTransform can generate large objects in terms of memory, hence adjust the limit for allowable object sizes within R 
```{r}
options(future.globals.maxSize = 4000 * 1024^2)
...
Iterating sctransform over each object in split_seurat_noMito
```{r}
 for (i in seq_along(split_seurat_noMito)){
  split_seurat_noMito[[i]] = SCTransform(split_seurat_noMito[[i]], vars.to.regress = c("mitoRatio")) # regressing out mitochondrial gene ratio as a factor
}
```

checking assays stored in objects
```{r}
split_seurat_noMito$ctrl@assays
```
checking assays stored in objects
```{r}
split_seurat_noMito$treat@assays
```
redo save the split_seurat object since it took lot of memory and ~ 1 min to generate

```{r}
# redo saving the sctransform output object
# saveRDS(split_seurat_noMito, "./data/split_seurat_noMito.rds") # commented out since saved once 
```

## Redo of Integration after regressing out mitoRatio
...
```{r}
# select most variable features to be used for integration
integ_features <- SelectIntegrationFeatures(object.list = split_seurat_noMito,
                                            nfeatures = 3000)
```
...
```{r}
# prepare the SCT list object for integration
split_seurat_noMito <- PrepSCTIntegration(object.list = split_seurat_noMito,
                                   anchor.features = integ_features)
```
### perform CCA, find best buddies / anchors, filter incorrect anchors
```{r}
# find best buddies / anchors
integ_anchors <- FindIntegrationAnchors(object.list = split_seurat_noMito,
                                        normalization.method = "SCT",
                                        anchor.features = integ_features)
```
### Integrate across conditions
```{r}
# integrate across conditions
seurat_integrated_noMito <- IntegrateData(anchorset = integ_anchors,
                                   normalization.method = "SCT")
```
### Visualizing the integrated data with PCA
```{r}
# PCA
seurat_integrated_noMito <- RunPCA(object = seurat_integrated_noMito)

```
```{r}
# plot PCA
PCAPlot(object = seurat_integrated_noMito,
        split.by = "sample")
```
...
```{r}
# run UMAP
seurat_integrated_noMito <- RunUMAP(seurat_integrated_noMito,
                             dims = 1:40,
                             reduction = "pca")
```
```{r}
# plot UMAP
DimPlot(seurat_integrated_noMito)
...
```{r}
# save integrated seurat object
# saveRDS(seurat_integrated_noMito, "./results/integrated_seurat_noMito.rds") # saved in macbookair
```


## Redo of Clustering foll; Integration redo
### Determining number of PCs to use
```{r}
# elbow / scree plot
ElbowPlot(seurat_integrated_noMito,
          ndims = 50)
```
We will use 50 PCs to determine clusters
...
```{r}
# K-nearest neighbors graph
seurat_integrated_noMito <- FindNeighbors(object = seurat_integrated_noMito,
                                   dims = 1:50)

#determine clusters at various resolutions
seurat_integrated_noMito <- FindClusters(object = seurat_integrated_noMito,
                                  resolution = c(0.4, 0.6, 0.8, 1.0, 1.4))
```

### looking at various resolutions
```{r}
seurat_integrated_noMito@meta.data %>% 
  View()
```
...
```{r}
# assigning identity of clusters
Idents(object = seurat_integrated_noMito) <- "integrated_snn_res.1.4"
```
...
```{r}
# assigning identity of clusters
Idents(object = seurat_integrated_noMito) <- "integrated_snn_res.1.0"
```
...
```{r}
# assigning identity of clusters
Idents(object = seurat_integrated_noMito) <- "integrated_snn_res.0.8"
```
...
```{r}
# assigning identity of clusters
Idents(object = seurat_integrated_noMito) <- "integrated_snn_res.0.6"
```
...
```{r}
# assigning identity of clusters
Idents(object = seurat_integrated_noMito) <- "integrated_snn_res.0.2"
```
...
```{r}
# assigning identity of clusters
Idents(object = seurat_integrated_noMito) <- "integrated_snn_res.0.4"
```
...
## Redo of QC for Clustering
### Distribution of cells per cluster in each condition
```{r}
# extracting identity and sample information to determine cells per cluster in each condition
n_cells <- FetchData(seurat_integrated_noMito,
                     vars = c("ident", "orig.ident")) %>% 
  count(ident, orig.ident) %>% 
  spread(ident, n)
...
FeaturePlot(seurat_integrated_noMito,
            reduction = "umap",
            features = metrics,
            pt.size = 0.4,
            order = TRUE,
...
pc_data <- FetchData(seurat_integrated_noMito, 
                     vars = columns)
```
...
umap_label <- FetchData(seurat_integrated_noMito,
...
Examine PCA results
```{r}
print(seurat_integrated_noMito[["pca"]], dims = 1:5, nfeatures = 5)
```

### Visualizing cd19 positive cells
```{r}
# select the RNA counts slot to be default assay
DefaultAssay(seurat_integrated_noMito) <- "RNA"

# Normalize RNA data for visualization
seurat_integrated_noMito <- NormalizeData(seurat_integrated_noMito, verbose = FALSE)
```

### B lymphocyte markers CD19 and CD79A
```{r}
FeaturePlot(seurat_integrated_noMito,
            reduction = "umap",
            features = c("CD19", "CD79A"),
            order = TRUE,
            min.cutoff = "q10",
            label = TRUE)
```

## Identification of conserved markers in both conditions

Done using FindConservedMarkers 
...
### Setting default assay to use normalized not integrated data
```{r}
# setting default assay
DefaultAssay(seurat_integrated_noMito) <- "RNA"
```

### Defining threshold for identifying marker
...
cluster0_conserved_markers <- FindConservedMarkers(seurat_integrated_noMito,
                                                   ident.1 = 0,
                                                   grouping.var = "sample",
                                                   only.pos = TRUE,
...
```{r}
cluster0_conserved_markers <- cluster0_conserved_markers %>% 
...
get_conserved <- function(cluster){
  FindConservedMarkers(seurat_integrated_noMito,
                       ident.1 = cluster,
                       grouping.var = "sample",
                       only.pos = TRUE) %>% 
    rownames_to_column(var = "gene") %>% 
...
### Plot top 5 cluster0 markers in all clusters
```{r}
#cluster0 marker expression across clusters
FeaturePlot(object = seurat_integrated_noMito,
            features = c("IGLC2", "ADTRP", "ZFP36L2","CXXC5","VPREB3"),
            order = TRUE,
            min.cutoff = "q10",
            label = TRUE,
            repel = TRUE)
...
```{r}
#fibroblast marker expression across clusters
FeaturePlot(object = seurat_integrated_noMito,
            features = c("CXCL8", "PDGFRB", "PDPN"),
            order = TRUE,
            min.cutoff = "q10",
            label = TRUE,
            repel = TRUE)
...
VlnPlot(seurat_integrated_noMito,
        features = c("CD19"))
```

### violin plot for VPREB3 marker
```{r}
# distribution of vpreb3 across clusters
VlnPlot(seurat_integrated_noMito,
        features = c("VPREB3"))
```

### violiin plot for IGLC2 marker
```{r}
# distribution of IGLC2
VlnPlot(seurat_integrated_noMito,
        features = c("IGLC2"))
```

### violin plot for ACTB (top for cluster 1)
```{r}
VlnPlot(seurat_integrated_noMito,
        features = c("ACTB"))
```

### violin plot for NEAT1 (top for cluster 2)
```{r}
VlnPlot(seurat_integrated_noMito,
        features = c("NEAT1"))
```

### violin plot for CXCR4 (#1 marker for cluster 6)
```{r}
VlnPlot(seurat_integrated_noMito,
        features = c("ATF5"))
```

### violin plot for  (cluster 6)
```{r}
VlnPlot(seurat_integrated_noMito,
        features = c("CEBPB"))
```

### violin plot for (top for cluster 6)
```{r}
VlnPlot(seurat_integrated_noMito,
        features = c("IER5"))
```
...
violin plot for (top for cluster 7)
```{r}
VlnPlot(seurat_integrated_noMito,
        features = c("MX1"))
```
