...
The data provided is a "filtered_feature_bc_matrix". The "raw_feature_bc_matrix" has not been provided. Presumably, this data was filtered by Cellranger once already.
...
Each cell ID has a cll_cntrl_ or cll_treat_ prefix as we had specified when we merged the Seurat objects. We can use this prefix to create a new column indicating which condition each cell is classified under. We will call this column sample:
```{r}
